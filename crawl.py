...
def grok_discover():
    ...
    queries = [
        (
            f"What are the top trending topics and news stories on X right now related "
            f"to {topic}? Check the Explore page, Trending tab, and top news. "
    ...
    for i, query in enumerate(queries):
        ...
        try:
            ...
            print(f"  Grok x_search query {i + 1} error: {e}", file=sys.stderr)
            continue
...
def search_x_api(query, max_pages=3, sort_order="recency"):
...
def crawl_x():
    ...
    # --- X API keyword search (relevancy first, then recency) ---
    query = _build_query()
    print(f"  [X API] Keyword search: {query}", file=sys.stderr)

    api_added = 0
    for sort in ["relevancy", "recency"]:
        raw_search, search_users = search_x_api(query, sort_order=sort)
        added = 0
        for raw in raw_search:
            if raw["id"] not in seen_ids:
                ...
                added += 1
        api_added += added
        print(f"  [X API] {sort}: +{added} new ({len(raw_search)} fetched)", file=sys.stderr)

    print(f"  [X API] Total added: {api_added}", file=sys.stderr)
