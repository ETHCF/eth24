...
1. **Crawl** - Run `python3 crawl.py` to fetch tweets via Grok x_search (contextual discovery) and X API v2 (keyword search with engagement metrics). Requires `X_BEARER_TOKEN`, `XAI_API_KEY`, and optionally `ANTHROPIC_API_KEY` environment variables. Output: `output/YYYY-MM-DD/crawled.json`
